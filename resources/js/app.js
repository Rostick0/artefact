...
(function () {
    const visualizationSlider = document.querySelector(
...
(function () {
    ...
    portfolioFilterItems.forEach((item) => {
        item.onclick = () => {
            document
                .querySelectorAll(".portfolio__filter_item._active")
                .forEach((item) => classRemove(item, "_active"));
            document
                .querySelectorAll(".portfolio-item._active")
                .forEach((item) => classRemove(item, "_active"));
            ...
            portfolioItems.forEach((elem) => {
                ...
                if (
                    item.getAttribute("data-id") != elem.getAttribute("data-id")
                )
                    return;
...
(function () {
    const needVisualizationContainer = document.querySelector(
        ".need-visualization__container"
    );
    if (!needVisualizationContainer) return;

    observerOnce(document.querySelector(".need-visualization"), () =>
        classAdd(needVisualizationContainer, "_visible")
    );
})();
...
(function () {
    ...
    observerOnce(document.querySelector(".about__description"), () =>
        classAdd(aboutBtn, "_visible")
    );
})();

(function () {
    const calculatorSwitchItems = document.querySelectorAll(
        ".calculator__switch_item"
    );
    ...
    const [calculatorSwitch, setCalculatorSwitch] = useState(
    ...
    const updateCalculatorAmount = (value = null) => {
        let amountText =
            value ??
            +document.querySelector(".calculator-grid-item__input")?.value;
        if (isNaN(amountText)) return (calculatorAmount.textContent = "...");

        calculatorAmount.textContent = calculator(
            calculatorSwitch(),
            key(),
            counter(),
            amountText
    ...
    const calculatorGridItemInput = document.querySelector(
        ".calculator-grid-item__input"
    );
    ...
    const initInputs = () => {
        const calculatorGridItemRadios =
            document.querySelectorAll(".calculator__radio");
        const calculatorGridItemRadiosValues = document.querySelectorAll(
            ".calculator__radio_value"
        );
        ...
        // setCounter(
        //     +(calculatorGridItemRadiosValues[0].querySelector('.radio__input').value)
        // )
        ...
        const calculatorGridItemCheckboxs = document.querySelectorAll(
            ".calculator__radio_value"
        );

        calculatorGridItemRadiosValues?.forEach((elem) => {
            elem.onclick = (e) => {
                setCounter(e.target.value);
                // setCounter
                // setCount(
                //     [...document.querySelectorAll('.calculator__radio_value .radio__input:checked')]?.length
                // );
                updateCalculatorAmount();
            };
        });
    ...
    const updateCalcForm = (value) => {
        ...
        document.querySelector(".calculator-grid-item__radios").innerHTML =
            edit?.[value]?.radions;
        document.querySelector(".calculator-grid-item__checkboxs").innerHTML =
            edit?.[value]?.checkboxs;
    };

    const calculatorSwitchInput = document.querySelectorAll(
        ".calculator__switch_input"
    );
...
(function () {
    const adminFormEditorImage = document.querySelectorAll(
        ".admin-form-editor__image"
    );

    adminFormEditorImage.forEach((elem) => {
        const adminFormEditorImageClose = elem.querySelector(
            ".admin-form-editor__image_close"
        );

        adminFormEditorImageClose.onclick = () =>
